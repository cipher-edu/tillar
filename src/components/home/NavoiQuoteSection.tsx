    ...
    >
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <span
              className={`text-[10px] md:text-[11px] font-black uppercase tracking-[0.35em] font-ui ${
                isRoyal ? 'text-amber-400/90' : 'text-amber-800'
              }`}
            >
              {L(quote.sourceNote)}
            </span>
          ...
          >
            ...
            <div
              className={`absolute top-0 left-0 w-12 h-12 border-t-2 border-l-2 rounded-tl-[2.5rem] pointer-events-none ${
                isRoyal ? 'border-amber-400/40' : 'border-amber-500/50'
              }`}
            />
            <div
            ...
            <Quote
              className={`absolute top-6 right-6 w-14 h-14 md:w-16 md:h-16 opacity-20 ${
                isRoyal ? 'text-amber-400' : 'text-amber-600'
              }`}
              strokeWidth={1}
              aria-hidden
            />
            ...
            <blockquote className="relative">
                ...
                {quote.lines.map((line, i) => (
                  <motion.p
                    ...
                    className={`font-serif-classic italic leading-relaxed text-xl sm:text-2xl md:text-[1.65rem] ${
                      isRoyal ? 'text-amber-50/95' : 'text-slate-800'
                    }`}
                  >
                    {L(line)}
                  </motion.p>
              ...
              <footer className="mt-8 md:mt-10">
                <div className="flex items-center justify-center gap-3 mb-4">
                  <span className={`h-px w-10 ${isRoyal ? 'bg-amber-500/50' : 'bg-amber-400/60'}`} />
                  <span className={isRoyal ? 'text-amber-400/70' : 'text-amber-600/80'}>❦</span>
                  <span className={`h-px w-10 ${isRoyal ? 'bg-amber-500/50' : 'bg-amber-400/60'}`} />
                </div>
                <cite
                  className={`not-italic font-classic text-lg md:text-xl tracking-wide ${
                    isRoyal ? 'text-amber-300' : 'text-amber-900'
                  }`}
                >
                  — {L(quote.attribution)}
                </cite>
          ...
          {/* Zamonaviy dunyo / tamaddun izohi */}
          <motion.p
            initial={{ opacity: 0 }}
