  ...
  if (detail) {
    ...
    return (
      <PageShell title={L(detail.name)}>
        <div className="mb-6">
          <Link to="/fakultet/tuzilma" className="text-[11px] font-black uppercase tracking-widest text-amber-800 font-ui">
            ← {t('back')}
          </Link>
        </div>
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="glass-card p-10 rounded-[3rem] border-amber-200">
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {staff.map((p) => (
                  <Link key={p.id} to={personPath(p)} className="flex items-center gap-4 p-4 glass-card rounded-[2rem] border-amber-100 hover:border-amber-300">
          ...
          {head && (
            <div className="glass-card p-8 rounded-[3rem] border-amber-200 h-fit">
              <p className="text-[10px] font-black uppercase tracking-widest text-amber-700 mb-4 font-ui">{t('structure_head')}</p>
              <Link to={personPath(head)} className="block text-center group">
                <img src={head.photo} alt="" className="w-32 h-32 rounded-[2rem] object-cover mx-auto mb-4 border-2 border-amber-200" />
                <h3 className="font-classic text-2xl group-hover:text-amber-800">{L(head.name)}</h3>
        ...
        {departments.map((dep) => {
          ...
          return (
            <Link
              ...
              className="glass-card p-10 md:p-12 rounded-[3rem] border-amber-200 hover:shadow-2xl transition-all group relative overflow-hidden"
            >
              <div className="absolute inset-0 royal-gradient opacity-0 group-hover:opacity-[0.03] transition-opacity" />
              <h3 className="text-2xl md:text-3xl font-classic text-slate-900 mb-4 group-hover:text-amber-800 transition-colors">
                {L(dep.name)}
              </h3>
